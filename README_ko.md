...
- **선택 번역 (⌘⇧T)** — 텍스트를 드래그하면 자동 복사 + 번역 (손쉬운 사용 권한 필요)
- **화면 캡처 번역 (⌘⇧L)** — 현재 활성 창을 캡처하고 Vision OCR로 텍스트 추출 후 번역 (화면 기록 권한 필요)
- **퀵 액션** — 번역 / 요약 / 설명 버튼
- **모델 선택** — 채팅 모델과 퀵 액션 모델을 분리 선택 (sonnet, haiku, opus)
- **시스템 프롬프트** — 번역 스타일 커스텀 가능 (ex: IT 용어 원문 유지, 영어를 한글로 번역)
...
| 권한 | 용도 | 필수 여부 |
|------|------|-----------|
| 손쉬운 사용 | ⌘⇧T 드래그 텍스트 자동 추출 | 선택 (없으면 수동 복사 후 번역) |
| 화면 기록 | ⌘⇧L 화면 캡처 번역 | ⌘⇧L 사용 시 필수 (앱 재시작 필요) |

## 제한사항

- **Claude 전용** — [`claude -p`](https://code.claude.com/docs/ko/cli-reference) CLI를 사용하므로 Claude Code가 설치되어 있어야 합니다 ([Thariq's Post](https://x.com/trq212/status/2024212380142752025))
